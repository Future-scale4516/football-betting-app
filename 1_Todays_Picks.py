...
from results_tracker import log_picks_bulk
...
# ------------------------------------------------------------------ logging
st.divider()
st.subheader("Log these picks")
st.caption(
    f"Saves the {len(view)} pick(s) currently shown above to the Results page "
    "so they can be settled and scored later. Re-running the same day won't "
    "create duplicates. Narrow the filters first — logging everything makes "
    "the results log noisy and the hit-rate stats meaningless."
)

if len(view) > 40:
    st.warning(
        f"{len(view)} picks is a lot to track. Consider filtering to a single "
        "market or tier first."
    )

if st.button("Log these picks", type="primary"):
    added = log_picks_bulk(view, sel_date)
    if added:
        st.success(f"Logged {added} new pick(s) — see the Results page.")
    else:
        st.info("Nothing new to log; these are already in the results log.")
